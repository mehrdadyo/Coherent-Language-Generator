...
from modified_gpt2_generation import Modified_GPT2_Generation
...
MODEL_CLASSES = {
    "gpt2": (GPT2Config, Modified_GPT2_Generation, GPT2Tokenizer),
...
def train(args, train_dataset, model: PreTrainedModel, tokenizer: PreTrainedTokenizer, train_dataset_second, DP_classifier, NSP_model) -> Tuple[int, float]:
    """ Train the model """
    ...
    correct_mc_tensor_one = torch.ones(1, dtype=torch.float)
    correct_mc_tensor_one = correct_mc_tensor_one.to(args.device)
    ...
    for _ in train_iterator:
        ...
        for step, zipped_batch in enumerate(epoch_iterator):
            ...
            outputs = model(correct_input, lm_labels=correct_labels, precondition_embd = sentence_embed_1)
            ...
            accumulated_lm_loss += loss_lm.item()
            loss_second = 0.0
            
            if curr_batch_num >= 5:
                # Second loss: Run language generation and reinforcement learning starting from second epoch
                wrong_input = wrong_batch[0, :100]
                wrong_input = wrong_input.unsqueeze(0)
                wrong_input_cls = wrong_batch[0, :101]
                wrong_input_cls[-1] = tokenizer.cls_token_id
                wrong_input_cls = wrong_input_cls.unsqueeze(0)
                wrong_input_cls = wrong_input_cls.to(args.device)
                outputs = NSP_model(wrong_input_cls)
                hidden_cls = outputs[2]
                sentence_embed_cls = hidden_cls[:,-1]
                
    
                wrong_input = wrong_input.to(args.device)
                model.precondition_embd = sentence_embed_cls
                
                MAX_LEN_ = 500
                output_sequences = model.generate(
                    input_ids=wrong_input,
                    max_length=MAX_LEN_,
                    temperature=1.0,
                    top_k=50,
                    top_p=1.0,
                ...
                output_sequences_second_half = output_sequences[:, 100:].clone()
                output_sequences_second_half[:,-1] = tokenizer.cls_token_id
                outputs = NSP_model(output_sequences_second_half)
                hidden_cls = outputs[2]
                sentence_embed_second_half = hidden_cls[:,-1]
                
                if random.randint(0, 1) == 1:
                    outputs = DP_classifier(sentence_embed_cls, sentence_embed_second_half)
                else:
                    outputs = DP_classifier(sentence_embed_second_half, sentence_embed_cls)
            
                mc_logits = outputs[0]
                loss_fct = torch.nn.BCEWithLogitsLoss()
                loss_mc_rl = loss_fct(mc_logits.view(-1), correct_mc_tensor_one.view(-1))
                
                curr_mc_rl_val = loss_mc_rl.item()
                loss_running_average = 0.95 * loss_running_average + 0.05 * curr_mc_rl_val
                print("curr_mc_rl_val", curr_mc_rl_val)
                
                output_sequences_label = output_sequences.clone().to("cpu")
                for curr in output_sequences_label:
                    for idx, tk in enumerate(curr):
                        if idx < 100:
                            curr[idx] = -100
                output_sequences_label = output_sequences_label.to(args.device)
                outputs = model(output_sequences, lm_labels=output_sequences_label, precondition_embd=sentence_embed_cls)
    
                loss_lm_3 = outputs[0]
                
                SCALING = 1
                loss_second = SCALING * loss_lm_3 * (loss_running_average - loss_mc_rl)
                accumulated_mc_loss += curr_mc_rl_val
            ...
            SKIP_STEP = 25
            if (step % SKIP_STEP == 0):
                print(' iter %d, avg. lm_loss %.3f, avg. NSP_loss %.3f, avg. ppl %.3f ' % (step,
                                                                    accumulated_lm_loss / SKIP_STEP,
                                                                    accumulated_mc_loss / SKIP_STEP,
                                                                    math.exp(loss_lm.item() ),
                                                                    ), file=sys.stderr)
            ...
            if (step + 1) % args.gradient_accumulation_steps == 0:
                ...
                if args.local_rank in [-1, 0] and args.logging_steps > 0 and global_step % args.logging_steps == 0:
                    ...
                    if (
                        ...
                        results = evaluate(args, model, tokenizer, DP_classifier, NSP_model=NSP_model)
...
def evaluate(args, model: PreTrainedModel, tokenizer: PreTrainedTokenizer, DP_classifier, NSP_model, prefix="", ) -> Dict:
    ...
    eval_dataset = load_and_cache_examples(args, tokenizer, evaluate=True, doubling = True)
    eval_dataset_second = load_and_cache_examples(args, tokenizer, evaluate=True, second=True, doubling=True)
    ...
    for zipped_batch in tqdm(zip(eval_dataloader, eval_correct_dataloader, eval_wrong_dataloader), desc="Evaluating", total=len(eval_dataloader)):
        ...
        labels = labels.to(args.device)
        with torch.no_grad():
            outputs = model(second_input, lm_labels = labels, precondition_embd=sentence_embed_1)
        lm_loss = outputs[0]
...
def main():
    ...
    if args.model_name_or_path:
        ...
        NSP_model = Modified_GPT2_NSP.from_pretrained(
            args.model_name_or_path,
            from_tf=bool(".ckpt" in args.model_name_or_path),
            config=config,
            cache_dir=args.cache_dir,
    ...
    model.linear1_1.requires_grad_(True)
    model.linear1_2.requires_grad_(True)
    model.linear2.requires_grad_(True)
    for p in NSP_model.parameters():
        p.requires_grad = False

    model.to(args.device)
    NSP_model.to(args.device)
    ...
    ############# Freeze part of the model ############
    for p in DP_classifier.parameters():
        p.requires_grad = False   
    ...
    if args.do_train:
        ...
        global_step, tr_loss = train(args, train_dataset, model, tokenizer, train_dataset_second, DP_classifier = DP_classifier, NSP_model = NSP_model)
    ...
    if args.do_eval and args.local_rank in [-1, 0]:
        ...
        for checkpoint in checkpoints:
            ...
            model.to(args.device)
            result = evaluate(args, model, tokenizer, DP_classifier, prefix=prefix, NSP_model=NSP_model)
