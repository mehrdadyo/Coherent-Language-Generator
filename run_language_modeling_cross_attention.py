    ...
    def __init__(self, tokenizer: PreTrainedTokenizer, args, file_path: str, block_size=512, second=False, doubling=False):
        ...
        if os.path.exists(cached_features_file) and not args.overwrite_cache:
            ...
            if (second):
                ...
                last_token = tokenizer.sep_token_id
                

            if second:
                for i in range(block_size, len(tokenized_text) - block_size + 1, block_size):  # Truncate in block of block_size
                    self.examples.append(tokenizer.build_inputs_with_special_tokens(tokenized_text[i : i + block_size] + [last_token]))
            else:
                for i in range(0, len(tokenized_text) - 2 * block_size + 1, block_size):  # Truncate in block of block_size
                    self.examples.append(tokenizer.build_inputs_with_special_tokens(tokenized_text[i : i + block_size] + [last_token]))
...
def train(args, train_dataset, model: PreTrainedModel, tokenizer: PreTrainedTokenizer, train_dataset_second) -> Tuple[int, float]:
    """ Train the model """
    ...
    correct_mc_tensor = torch.ones(args.train_batch_size, dtype=torch.long)
    correct_mc_tensor = correct_mc_tensor.to(args.device)
    wrong_mc_tensor = torch.zeros(args.train_batch_size, dtype=torch.long)
    ...
    for _ in train_iterator:
        ...
        for step, zipped_batch in enumerate(epoch_iterator):
            ...
            # First loss : correct next sentence
            ...
            correct_labels[:, -1] = -100
            first_merged_inputs = torch.cat((inputs, correct_input), 1)
            first_merged_labels = torch.cat((labels, correct_labels), 1)
            
            first_merged_inputs = first_merged_inputs.to(args.device)
            first_merged_labels = first_merged_labels.to(args.device)
            #print(first_merged_inputs.shape)
            #print(first_merged_labels.shape)
            
            model.train()
            
            outputs = model(inputs, masked_lm_labels=labels) if args.mlm else model(first_merged_inputs, lm_labels=first_merged_labels, mc_labels = correct_mc_tensor)
            loss_mc = outputs[1]  # model outputs are always tuple in transformers (see doc)
            loss_lm = outputs[0]
            ...
            wrong_labels[:, -1] = -100
            second_merged_inputs = torch.cat((inputs, wrong_input), 1)
            second_merged_labels = torch.cat((labels, wrong_labels), 1)
            
            second_merged_inputs = second_merged_inputs.to(args.device)
            second_merged_labels = second_merged_labels.to(args.device)
            
            outputs = model(inputs, masked_lm_labels=labels) if args.mlm else model(second_merged_inputs, lm_labels=second_merged_labels, mc_labels = wrong_mc_tensor)
            loss_mc = outputs[1]  # model outputs are always tuple in transformers (see doc)
            
            #loss = loss_lm
            loss_second = SCALING * loss_mc
            #print("loss_mc: ", loss_mc.item())
            #print("loss_lm: ", loss_lm.item())
            accumulated_mc_loss += SCALING * loss_mc.item()
...
def evaluate(args, model: PreTrainedModel, tokenizer: PreTrainedTokenizer, prefix="") -> Dict:
    ...
    for zipped_batch in tqdm(zip(eval_dataloader, eval_correct_dataloader, eval_wrong_dataloader), desc="Evaluating", total=len(eval_dataloader)):
        ...
        with torch.no_grad():
            outputs = model(inputs, masked_lm_labels=labels) if args.mlm else model(first_merged_inputs)
...
def main():
    ...
    if args.do_train:
        ...
        global_step, tr_loss = train(args, train_dataset, model, tokenizer, train_dataset_second)
